...
def main ():
    ...
    st.title('Aleradev Data Sicence')
    ...
    if file is not None:
        st.subheader('Estatistica descritiva univariada')
        df = pd.read_csv(file)
        ...
        if barras:
            ...
            col_cat_barras = st.selectbox('Selecione a Coluna categorica: ',colunas_object,key = 'unique')
            st.markdown('Gráfico de barras de coluna  '+ str(col_cat_barras) + 'pela coluna ' + col_num_barras)
        ...
        if boxplot:
            ...
            col_cat_box = st.selectbox('Selecione a Coluna categorica: ',colunas_object,key = 'unique')
        ...
        if scatter:
            ...
            col_color = st.selectbox('Selecione a coluna para a cor', colunas)
