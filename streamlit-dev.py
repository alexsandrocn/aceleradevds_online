...
def main():
    ...
    opcoes = ('Dataframe', 'Ánalise exploratoria', 'Inputação de dados', 'Visualização dos dados')
    ...
    if file is not None:
        ...
        if (sidebarCategoria == opcoes[0]):
            ...
            percentual = st.slider(
                'Escolha o limite de percentual faltante para as colunas que você deseja inputar os dados', min_value=0,
                max_value=100)
            lista_colunas = list(
                dados_faltante[dados_faltante['NA %'] <= percentual][dados_faltante['tipos'] != 'object']['atributos'])
